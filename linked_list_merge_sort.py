...
def split(linked_list):
    ...
    if linked_list == None or linked_list.head == None:
        # assign all the left half and none to the right
        left_half = linked_list
        right_half = None

        return left_half, right_half
    else:
        # for non empty linked lists
        size = linked_list.size()
        mid = size // 2
        # next need to find the node at this index
...
def merge(left, right):
    """
    Merges two linked links, sorting by data in nodes
